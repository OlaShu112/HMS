...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    validate($specialization, 'Please enter your specializations!');
    ...
    if ($user['email'] !== $email) {
        ...
        mysqli_query($conn, "UPDATE `doctor` SET `specialization`='$specialization' WHERE `user_id`=$user_id");
        // Image process
...
if (move_uploaded_file($_FILES["photo"]["tmp_name"], $targetFile)) {
    ...
    $_SESSION['img'] = $uniqueFileName;
    exit(json_encode(array('status' => 'success', 'msg' => 'Your profile is updated!','img'=>$url.'assets/data/'.$uniqueFileName)));
}else{
    exit(json_encode(array('status' => 'error', 'msg' => 'Something went wrong during uploading image but all other data is updated!')));
}
}
exit(json_encode(array('status' => 'success', 'msg' => 'Your profile is updated!','img'=>'a')));
