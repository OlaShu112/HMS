...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    validate($department,'Please enter department!');
    validate($shift,'Please select shift!');
...
$create_user = mysqli_query($conn, "INSERT INTO `users` ( `name`,`role_id`, `email`,`phone`, `username`, `password`, `created_at`) VALUES ('$name', 5,'$email',$phone,'$username', '$hash', current_timestamp())");
$data = mysqli_fetch_assoc(mysqli_query($conn,"Select * from users where email='$email'"));
$user_id = $data['id'];
$create_patient = mysqli_query($conn, "INSERT INTO `nurse` ( `user_id`,`shift`,`department`) VALUES ($user_id,'$shift','$department')");
            exit(json_encode(array('status' => 'success', 'msg' => 'Nurse is successfully created!')));
        }else{
