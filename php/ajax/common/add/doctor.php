...
$create_user = mysqli_query($conn, "INSERT INTO `users` ( `name`,`role_id`, `email`,`phone`, `username`, `password`, `created_at`) VALUES ('$name', 3,'$email',$phone,'$username', '$hash', current_timestamp())");
$data = mysqli_fetch_assoc(mysqli_query($conn,"Select * from users where email='$email'"));
$user_id = $data['id'];
$create_patient = mysqli_query($conn, "INSERT INTO `doctor` ( `user_id`,`specialization`,`gender`) VALUES ($user_id,'$specialization','$gender')");
            exit(json_encode(array('status' => 'success', 'msg' => 'Doctor is successfully created!')));
        }else{
