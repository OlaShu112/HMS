...
$create_user = mysqli_query($conn, "INSERT INTO `users` ( `name`,`role_id`, `email`,`phone`, `username`, `password`, `created_at`) VALUES ('$name', 7,'$email',$phone,'$username', '$hash', current_timestamp())");
$data = mysqli_fetch_assoc(mysqli_query($conn,"Select * from users where email='$email'"));
$user_id = $data['id'];
            exit(json_encode(array('status' => 'success', 'msg' => 'Hospital Staff is successfully created!')));
        }else{
