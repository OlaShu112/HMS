...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($user['email'] !== $email) {
        ...
        mysqli_query($conn, "UPDATE `patient` SET `dob`='$dob', `address`='$address' WHERE `user_id`=$user_id");
        // Image process
...
if (move_uploaded_file($_FILES["photo"]["tmp_name"], $targetFile)) {
    ...
    exit(json_encode(array('status' => 'success', 'msg' => 'Patient profile is updated!','img'=>$url.'assets/data/'.$uniqueFileName)));
}else{
    exit(json_encode(array('status' => 'error', 'msg' => 'Something went wrong during uploading image but all other data is updated!')));
}
}
exit(json_encode(array('status' => 'success', 'msg' => 'Patient profile is updated!','img'=>'a')));
