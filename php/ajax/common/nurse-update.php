...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($user['email'] !== $email) {
        ...
        mysqli_query($conn, "UPDATE `nurse` SET `department`='$department', `shift`='$shift' WHERE `user_id`=$user_id");
        // Image process
...
if (move_uploaded_file($_FILES["photo"]["tmp_name"], $targetFile)) {
    ...
    exit(json_encode(array('status' => 'success', 'msg' => 'Nurse profile is updated!','img'=>$url.'assets/data/'.$uniqueFileName)));
}else{
    exit(json_encode(array('status' => 'error', 'msg' => 'Something went wrong during uploading image but all other data is updated!')));
}
}
exit(json_encode(array('status' => 'success', 'msg' => 'Nurse profile is updated!','img'=>'a')));
