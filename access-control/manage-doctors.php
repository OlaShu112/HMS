...
<?php
$data = mysqli_query($conn,"SELECT * FROM `users` WHERE `role_id`=3 order by id desc");
?>
<section class="bg-white ">
            <div class="py-8 px-4 mx-auto max-w-screen-xl text-start lg:py-16 lg:px-12">
        
                <h1 class="mb-4 text-2xl font-extrabold tracking-tight leading-none text-gray-900 md:text-3xl ">Manage Doctors</h1>
  ...
  <td class="px-6 py-4 whitespace-nowrap text-end text-sm font-medium">
    <button id="dropdownMenuIconButton" data-dropdown-toggle="dropdownDots<?=$sno?>" class="inline-flex items-center p-2 text-sm font-medium text-end text-gray-900 bg-white rounded-lg hover:bg-gray-100 focus:ring-4 focus:outline-none  focus:ring-gray-50 " type="button">
      ...
      <div id="dropdownDots<?=$sno?>" class="z-10 hidden bg-white divide-y divide-gray-100 rounded-lg shadow  ">
          ...
          <li>
              <a href="<?=$url?>access-control/doctor-details?id=<?=$row['id']?>" class="block px-4 py-2 hover:bg-gray-100 ">View</a>
            </li>
            <li>
              <a href="<?=$url?>access-control/doctor-edit?id=<?=$row['id']?>" class="block px-4 py-2 hover:bg-gray-100 ">Edit</a>
            </li>
            <li>
              <a href="<?=$url?>php/ajax/common/delete/doctor.php?id=<?=$row['id']?>" class="block px-4 py-2 hover:bg-gray-100 ">Delete</a>
            </li>
          </ul>
      </div>
  </td>
