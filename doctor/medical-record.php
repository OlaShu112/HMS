...
<?php
$data = mysqli_query($conn,"SELECT * FROM `medical_records` WHERE `staff_id`=$_SESSION[user_id] order by id desc");
?>
</style>
...
<section class="bg-white ">
            <div class="py-8 px-4 mx-auto max-w-screen-xl text-start lg:py-16 lg:px-12">
              ...
              <div class="flex justify-between items-center">
                ...
                <div><a href="<?=$url?>doctor/add-medical-record" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 focus:outline-none">Add Medical Record</a></div>
              </div>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-end text-sm font-medium">
                                  <button id="dropdownMenuIconButton" data-dropdown-toggle="dropdownDots<?=$sno?>" class="inline-flex items-center p-2 text-sm font-medium text-end text-gray-900 bg-white rounded-lg hover:bg-gray-100 focus:ring-4 focus:outline-none  focus:ring-gray-50 " type="button">
                                    ...
                                    <div id="dropdownDots<?=$sno?>" class="z-10 hidden bg-white divide-y divide-gray-100 rounded-lg shadow  ">
                                        ...
                                        <li>
                                            <a href="<?=$url?>doctor/medical-record-details?id=<?=$row['id']?>" class="block px-4 py-2 hover:bg-gray-100 ">View</a>
                                          </li>
                                          <li>
                                            <a href="<?=$url?>doctor/medical-record-edit?id=<?=$row['id']?>" class="block px-4 py-2 hover:bg-gray-100 ">Edit</a>
                                          </li>
                                          <li>
                                            <a href="<?=$url?>php/ajax/common/delete/medical-record.php?id=<?=$row['id']?>" class="block px-4 py-2 hover:bg-gray-100  ">Delete</a>
                                          </li>
                                        </ul>
                                    </div>
                                </td>
